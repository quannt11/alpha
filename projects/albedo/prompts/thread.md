...
2. **Implement** on your branch (`lab/{{project}}-<thread>-<topic>`) in the repo you work on; commit locally,
   never push. If the eval rules changed (World news in your prompt), check your scorer and harness first.
3. **Run it on a GPU**:
...
5. **Report to the Researcher** when you have something that changes the picture (a clear result, a
   ...
   eval rules is a claim: `lab thread claim --text <evidence file>` (the Analyst red-teams it).
