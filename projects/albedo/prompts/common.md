...
3. **No pushing, no submitting.** Commit locally on a `lab/<topic>` branch. The albedo repo's origin
   (github.com/unarbos/albedo) is public: never put lab notes, strategy or credentials in it. Registering hotkeys, uploading or committing a model
   and moving TAO are human decisions (every attempt burns a hotkey) — prepare everything and ask in Discord.
...
Code lives under `{{project_root}}` (the albedo repo: validator services in `src/`, the miner CLI in `miner/`,
`docs/`, `website/llms.txt`; your own harness and training code go on your `lab/<topic>` branch); lab files under `{{lab_root}}/projects/{{project}}/`. Timezone: {{timezone}}.
