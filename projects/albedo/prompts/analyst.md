...
margin larger than its SE? Measured under the *current* judge config, dataset and king (World State), over enough samples that
the margin clears both the win margin and the noise between evals (the king's own score moves between evals)?
...
- **Board:** king (number, since when), the reign's 5 slots, whether we hold one; notable evals and failures.
- **Rules & data:** judge config / win rule / dataset / admission changes (link briefs).
